...
public class BallControl : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        areaAffector = GameObject.FindGameObjectWithTag("AreaAffactor");
        areaAffector.transform.position = this.gameObject.transform.position;
    ...
    void Update()
    {
        if(rgBody.velocity.magnitude < 0.1 && rgBody.velocity == Vector3.zero)
        {
            if (!ballIsStatic)   //if velocity is zero and ballIsStatic is false
            {
